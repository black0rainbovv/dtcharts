...
void QCPAxis::rescale(bool onlyVisiblePlottables)
{
  ...
  for (int i=0; i<p.size(); ++i)
  {
    ...
    {
      plottableRange.lower = p.at(i)->getValueRange(currentFoundRange, signDomain).lower*0.9;
      plottableRange.upper = p.at(i)->getValueRange(currentFoundRange, signDomain).upper*1.1;
    }
...
void QCPAxisRect::zoomEvent(double scalex, double scaley, QPoint pos)
{
    ...
    if (mRangeZoomVertAxis.data())
    {
        mRangeDragVertAxis.data()->moveRange(mRangeZoomVertAxis.data()->pixelToCoord(pos.y()) - mRangeZoomVertAxis.data()->pixelToCoord(this->center().y()));
...
QCPRange QCPFinancial::getValueRange(bool &foundRange, QCPAbstractPlottable::SignDomain inSignDomain) const
{
  ...
  QCPFinancialDataMap::const_iterator lower, upper; // note that upper is the actual upper point, and not 1 step after the upper point
   ...
   if(upper != mData->constEnd())
       upper++;
  
  QCPFinancialDataMap::const_iterator it = lower;
  while (it != upper)
  {
    // high:
